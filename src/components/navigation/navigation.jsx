...
export function Navigation() {
    ...
    const linkList = [
        ...
        {name:"Property", isActive:false, link:"properties"},
        ...
        <div className=' md:flex justify-between items-center  fixed w-full z-50 bg-cream-1 p-4 md:p-10 top-0 text-zinc-500 '>
            ...
            <div className={`flex flex-row items-center justify-center md:relative fixed bg-cream-1 md:z-auto z-[20] right-0 w-full md:w-auto h-screen md:h-auto transition-all duration-300 ease-linear ${closeBtn? "translate-x-[100%]" : "translate-x-[0]"} md:top-auto md:translate-x-0 top-0`}>
                ...
                <ul className="md:flex flex-col md:flex-row md:py-0 py-4 ">
                    {navLinks.map((item, index) => <li onClick={() => btnActiveHandler(item.name)} key={index} className={`cursor-pointer text-xl md:text-base  md:px-2 md:py-1 p-4 select-none ${item.isActive ? "bg-zinc-800 " : "bg-transparent"}`} ><NavLink to={item.link
